<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="投标文件" prop="toubiaowenjian">
          <file-upload
          tip="点击上传投标文件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.toubiaowenjian" label="投标文件" prop="toubiaowenjian">
            <el-button type="text" size="small" @click="download(ruleForm.toubiaowenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.toubiaoneirong = this.ruleForm.toubiaoneirong.replace(reg,'../../../springbootxf481/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `toubiaoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.toubiaoxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
